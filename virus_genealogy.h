...
struct TriedToRemoveStemVirus : public std::invalid_argument {
    TriedToRemoveStemVirus()
        : std::invalid_argument("TriedToRemoveStemVirus"){};
};
  ...
  private:
    struct VirusNode {
        ...
        std::set<std::shared_ptr<VirusNode>,
                 std::owner_less<std::shared_ptr<VirusNode>>>
            children;
        std::set<std::weak_ptr<VirusNode>,
                 std::owner_less<std::weak_ptr<VirusNode>>>
            parents;
    ...
    std::shared_ptr<VirusNode>
    get_node(typename Virus::id_type const &id) const {
    ...
    // Helper function for remove, adds iterators of elements to remove to
    // passed vector.
    ...
    void remove_dfs(
        ...
        std::vector<typename std::map<typename Virus::id_type,
                                      std::shared_ptr<VirusNode>>::iterator>
            &nodes_to_delete) {
        ...
        for (auto child : (node_it->second)->children) {
            if (child.get()->parents_counter == 1) {
                nodes_to_delete.push_back(
                    viruses.find(child.get()->virus.get_id()));
